...
from json import load, dump # Importing the function to read and write json files.
import datetime # Importing the datetime module for real-time date and time information.
from dotenv import dotenv_values # Importing dotenv_values to read environment variables from a .env file.

# Load environment variables frome the .env file.
...
def RealtimeInformation():
    ...
    hour = current_date_time.strftime("%H") # Hous in 24-hours format.
...
# Function to modify the chatbot's response for the better formatting.
def AnswerModifier(Answer):
    lines = Answer.split('\n') # Split response into the lines.
    non_empty_lines = [line for line in lines if line.strip()] # Remove empty lines.
    modified_answer = '\n'.join(non_empty_lines) # Join the claned lines back together.
    return modified_answer

# Main chatbot function to handle user queries.
def ChatBot(Query):
    """ This function sends the user's query to the chatbot and retuerns the API's response. """
    ...
    try:
        ...
        for chunk in completion:
            if chunk.choices[0].delta.content: # check if there's content in the correct chunk.
                Answer += chunk.choices[0].delta.content # Append the content to the answer.
                
        Answer = Answer.replace("<\s>", "") # Clean up any unwanted tokens form the response.
